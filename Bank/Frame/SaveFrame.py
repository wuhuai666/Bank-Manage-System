from tkinter import *
from Bank.Function.SaveMoney import *
class SaveFrame(Frame):  # 继承Frame类
    ...
    def createPage(self):
        ...
        Label(self, text='存款金额: ', font=('华文行楷', 15)).grid(row=3, stick=W, pady=10)
        Entry(self, textvariable=self.money, font=('华文行楷', 15)).grid(row=3, column=1, stick=E)
        Button(self, text='存款', font=('华文行楷', 15),command=self.save).grid(row=6, column=1, stick=E, pady=10)
    count=3
    def save(self):
        # 局部变量
        global count
        ...
        save = SaveMoney(Idnumber, password,money)
        count1 = save.save(self.count)
        if count1 == 0:
            self.count = 3
        else:
            self.count = count1
