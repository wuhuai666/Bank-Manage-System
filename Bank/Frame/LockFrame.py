...
from Bank.Function.LockCard import LockCard


class LockFrame(Frame):  # 继承Frame类
    ...
    def createPage(self):
        ...
        Button(self, text='锁定银行卡', font=('华文行楷', 15),command=self.lock).grid(row=6, column=1, stick=E, pady=10)
    def lock(self):
        Idnumber = self.IdNumber.get()
        password = self.password.get()
        lock = LockCard(Idnumber,password)
        lock.Lock()
